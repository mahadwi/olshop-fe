...
export default function ConsignmentIndex() {
    ...
    const loadConsignmentObject = () => {
        ...
        Api.get('/consignment')
            .then((res) => {
                if (res) {
                    setConsignmentObject({...DATA_DUMMY,...res.data.data[0]})
                }
            }).finally(() => {
                setLoading(false)
            })
                ...
                <div className="consignment-wrapper">
                    ...
                    <div className='consignment-first-hero'>
                        <div className='consignment-first-hero-image'>
                            <img src={consignmentObject.sections?.at(0).image} alt='Hero' />
                        </div>
                        <div className='consignment-first-hero-detail'>
                            <div className='_title0'>{consignmentObject.sections?.at(0)['title'+suffix] ?? ''}</div>
                            <div className='_title1' dangerouslySetInnerHTML={{ __html: consignmentObject.sections?.at(0)['description'+suffix]?? ''}}></div>
