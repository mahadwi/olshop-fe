...
export default function ConsignmentIndex() {
    ...
    const loadConsignmentObject = () => {
        setLoading(true);
        const getConsignment = Api.get("/consignment").then(res => {
            if (res) {
                setConsignmentObject(res.data.data[0]);
                setSections(
                    res.data.data[0].sections.reduce((p, c) => {
                        p[c.section] = c;
                        return p;
                    }, {})
                );
        ...
        const getBrands = Api.get("/brand?is_valid=true").then(res => {
            if (res) {
                res.data.data.sort((a, b) => a.id - b.id);
                setBrands(res.data.data.slice(0, 4));
            }
        });
        const getCommission = Api.get("/commission").then(res => {
            if (res) {
                setCommissions(res.data.data);
            }
        });
        Promise.all([getConsignment, getBrands, getCommission]).finally(() => {
            setLoading(false);
        });
    };
            ...
            <ContainerComponent>
                ...
                <div className="consignment-wrapper">
                    <div className="title">
                        ...
                        <div
                            className="title1"
                            dangerouslySetInnerHTML={{ __html: consignmentObject["description" + suffix] }}
                        />
                    </div>
                    ...
                    <div className="consignment-about" id="consignment-about">
                        <div className="consignment-about-video">
                            <iframe
                                src={`https://www.youtube-nocookie.com/embed/${consignmentObject.sections
                                    ?.at(1)
                                    .link.split("/")
                                    .at(-1)}?si=A1ujlX7IktTEKpc2`}
                        ...
                        <div className="consignment-about-detail">
                            ...
                            <div
                                ...
                                dangerouslySetInnerHTML={{
                                    __html: consignmentObject.sections?.at(1)["description" + suffix]
                                }}
                            />
                        </div>
                    </div>
                    <div className="consignment-div-title" style={{ margin: "4rem 0" }}>
                        BRANDS ACCEPTED
                    </div>
                                                ...
                                                {c.cards.map((s, i) => {
                                                    return (
                                                        <div className="program-step-card">
                                                            <div className="program-step-card-icon">
                                                                <img src={s.icon} alt={"icon"} width={42} />
                                                            </div>
                                                            <div className="program-step-card-name">
                                                                {s["title" + suffix]}
                                                            </div>
                                                            <div
                                                                className="program-step-card-description"
                                                                dangerouslySetInnerHTML={{
                                                                    __html: s["description" + suffix]
                                                                }}
                                                            />
                                                            {s.loadmore_type ? (
                                                                <div>
